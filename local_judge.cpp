...
int main() {
  ...
  for (int level = 1; level <= 5; ++level) {
    ...
    // 步骤 0（初始状态）
    {
      ...
      step.value(0);
      step.key("board");
      step.array_2d_int(board_to_int(board));
      step.key("path");
      step.start_array();
      step.end_array();
      step.end_object();
    ...
    for (int step = 1; step <= STEPS_PER_LEVEL; ++step) {
      ...
      Board new_board = board.preview(path);
      {
        ...
        step_builder.key("step");
        step_builder.value(step);
        step_builder.key("score");
        step_builder.value(score);
        step_builder.key("path");
        step_builder.array_pair(path);
        step_builder.key("board");
        step_builder.array_2d_int(board_to_int(new_board));
    ...
    level_builder.key("level");
    level_builder.value(level);
    level_builder.key("N");
    level_builder.value(N);
    level_builder.key("seed");
    level_builder.value(SEED);
    level_builder.key("total_score");
    level_builder.value(level_score);
  ...
  final_json.key("seed");
  final_json.value(SEED);
  final_json.key("final_score");
  final_json.value(grand_total);
